...
export class CanvasClass {
  ...
  previousTool = "";
  private onShapeComplete?: (shape: any) => void;

  setOnShapeComplete(callback: (shape: any) => void) {
  ...
  private handleMouseMove = (e: MouseEvent) => {
    ...
    if (this.clicked) {
      ...
      if (this.clicked && this.selectedTool === "pencile") {
        ...
        const tempStroke = new FreehandClass(
          this.currentStroke,
          this.strokeColor,
        );
      ...
      if (this.selectedTool === "rectangle") {
        const rectInstance = new RectangleClass(
          this.startX,
          this.startY,
          width,
          height,
      ...
      if (this.selectedTool === "circle") {
        ...
        const dy = this.clientY - centerY;
        const radius = Math.sqrt(dx * dx + dy * dy);
        const circleInstance = new CircleClass(
          centerX,
          centerY,
          radius,
          this.strokeColor,
      ...
      if (this.selectedTool === "ellipse") {
        ...
        const ellipseInstance = new EllipseClass(
          centerX,
          centerY,
          radiusX,
          radiusY,
  ...
  addShapeFromServer(shape: any) {
    switch (shape.type) {
      ...
      case "pencil":
        this.Shapes.push(new FreehandClass(shape.points, shape.strokeColor));
        break;
      case "ellipse":
        this.Shapes.push(
          new EllipseClass(
